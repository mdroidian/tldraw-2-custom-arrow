...
const HASH_PATTERN_ZOOM_NAMES: Record<string, string> = {};
...
export class CustomArrowTool2 extends StateNode {
  static override id = "opposes";
  static override initial = "idle";
  static override children = () => [Idle, Pointing];

  override shapeType = "opposes";
}

declare const newArrowShapeProps: {
  labelColor: EnumStyleProp<
    | "black"
    | "blue"
    | "green"
    | "grey"
...
declare type ShapePropsType<Config extends Record<string, T.Validatable<any>>> =
  Expand<{
    [K in keyof Config]: T.TypeOf<Config[K]>;
  }>;
type TLArrowShapeNewProps = ShapePropsType<typeof newArrowShapeProps>;
type TLArrowShapeNew = TLBaseShape<"opposes", TLArrowShapeNewProps>;

/** @public */
export class CustomArrowUtil2 extends ShapeUtil<TLArrowShapeNew> {
  static override type = "opposes" as const;
  //   static override props = arrowShapeProps;

  override canEdit = () => true;
  override canBind = () => false;
  override canSnap = () => false;
  override hideResizeHandles: TLShapeUtilFlag<TLArrowShapeNew> = () => true;
  override hideRotateHandle: TLShapeUtilFlag<TLArrowShapeNew> = () => true;
  override hideSelectionBoundsBg: TLShapeUtilFlag<TLArrowShapeNew> = () => true;
  override hideSelectionBoundsFg: TLShapeUtilFlag<TLArrowShapeNew> = () => true;

  override getDefaultProps(): TLArrowShapeNew["props"] {
    return {
      dash: "draw",
      size: "m",
      fill: "none",
  ...
  getGeometry(shape: TLArrowShapeNew) {
  ...
  private getLength(shape: TLArrowShapeNew): number {
    const info = this.editor.getArrowInfo(shape)!;
  ...
  override getHandles(shape: TLArrowShapeNew): TLHandle[] {
    const info = this.editor.getArrowInfo(shape)!;
  ...
  override onHandleDrag: TLOnHandleDragHandler<TLArrowShapeNew> = (
    shape,
    { handle, isPrecise }
  ) => {
    ...
    const next = deepCopy(shape) as TLArrowShapeNew;
    ...
    const target = this.editor.getShapeAtPoint(point, {
      ...
      filter: (targetShape) => {
        return (
          ...
          this.editor.getShapeUtil(targetShape).canBind(targetShape)
        );
      },
    });
    ...
    if (
      next.props.start.type === "binding" &&
      next.props.end.type === "binding"
    ) {
      if (next.props.start.boundShapeId === next.props.end.boundShapeId) {
        if (
          Vec.Equals(
            next.props.start.normalizedAnchor,
            next.props.end.normalizedAnchor
          )
        ) {
  ...
  override onTranslateStart: TLOnTranslateStartHandler<TLArrowShapeNew> = (
    shape
  ) => {
    ...
    if (
      ...
      (endBindingId &&
        (selectedShapeIds.includes(endBindingId) ||
          this.editor.isAncestorSelected(endBindingId)))
    ) {
      return;
    }
    ...
    for (const handleName of [
      ...
      result = {
        ...
        props: {
          ...shape.props,
          [handleName]: { ...terminal, isPrecise: true },
        },
      };
    }
  ...
  override onTranslate?: TLOnTranslateHandler<TLArrowShapeNew> = (
    initialShape,
    shape
  ) => {
  ...
  override onResize: TLOnResizeHandler<TLArrowShapeNew> = (shape, info) => {
    const { scaleX, scaleY } = info;

    const terminals = getArrowTerminalsInArrowSpace(this.editor, shape);

    const { start, end } = deepCopy<TLArrowShapeNew["props"]>(shape.props);
  ...
  override onDoubleClickHandle = (
    shape: TLArrowShapeNew,
    handle: TLHandle
  ): TLShapePartial<TLArrowShapeNew> | void => {
    switch (handle.id) {
  ...
  component(shape: TLArrowShapeNew) {
  ...
  indicator(shape: TLArrowShapeNew) {
    const { start, end } = getArrowTerminalsInArrowSpace(this.editor, shape);
  ...
  override onEditEnd: TLOnEditEndHandler<TLArrowShapeNew> = (shape) => {
    const {
      id,
      type,
      props: { text },
    ...
    if (text.trimEnd() !== shape.props.text) {
      this.editor.updateShapes<TLArrowShapeNew>([
        {
          id,
          type,
          props: {
  ...
  //   override toSvg(shape: TLArrowShapeNew, ctx: SvgExportContext) {
  ...
  //       if (labelGeometry) {
  //         const labelMask = document.createElementNS(
  //           "http://www.w3.org/2000/svg",
  //           "rect"
  //         );
